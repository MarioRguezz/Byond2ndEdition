         ...
         if(isset($user)) {
             ...
             foreach($cursos as $curso){
              foreach($curso->temas as $tema){
                ...
                $subtemasvistos = SubtemaVisto::all()->where('id_Curso','=', $curso->id_Curso)->where('Mat_Alumno','=', $user->alumno->Mat_Alumno)->where('Visto','!=','0');
                $user->alumno->subtemasVistos = count($subtemasvistos);
                if($curso->totalSubtemas == null ){
                  $curso->totalSubtemas = 0;
                }else{
                $user->alumno->totalSubtemas = $curso->totalSubtemas;
              }
              if($user->alumno->subtemasVistos != 0){

                if($user->alumno->totalSubtemas > 0) {
                  $curso->porcentaje =  ($user->alumno->subtemasVistos*100) / $user->alumno->totalSubtemas;
                }
              }else{
                $curso->porcentaje = 0;
              }
